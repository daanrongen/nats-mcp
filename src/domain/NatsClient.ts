...
export interface NatsClientService {
  // Core
  readonly publish: (subject: string, payload: string) => Effect.Effect<void, NatsError>;
  readonly request: (
    subject: string,
    payload: string,
    timeoutMs?: number,
  ...
  readonly streamCreate: (config: StreamConfig) => Effect.Effect<StreamInfo, NatsError>;
  ...
  readonly kvPut: (bucket: string, key: string, value: string) => Effect.Effect<void, NatsError>;
  readonly kvDelete: (bucket: string, key: string) => Effect.Effect<void, NatsError>;
  readonly kvListKeys: (bucket: string) => Effect.Effect<string[], NatsError>;
  readonly kvHistory: (bucket: string, key: string) => Effect.Effect<KvEntry[], NatsError>;

  // Server
  readonly serverInfo: () => Effect.Effect<ServerInfo, NatsError>;
}

export class NatsClient extends Context.Tag("NatsClient")<NatsClient, NatsClientService>() {}
